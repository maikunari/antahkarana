...
def load_transport_settings(environ: Mapping[str, str]) -> TransportSettings:
    """Read ANTAHKARANA_TRANSPORT, ANTAHKARANA_HOST and ANTAHKARANA_PORT; unset or empty means default.
    ...
    """
    transport = (environ.get("ANTAHKARANA_TRANSPORT") or "stdio").strip().lower()
    ...
    host = (environ.get("ANTAHKARANA_HOST") or "").strip() or DEFAULT_HOST
    raw_port = (environ.get("ANTAHKARANA_PORT") or "").strip()
    if not raw_port:
        return TransportSettings(transport, host)
    try:
        port = int(raw_port)
    except ValueError:
        port = 0
    ...
    return TransportSettings(transport, host, port)
