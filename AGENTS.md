...
- Secrets (`OPENROUTER_API_KEY`, optional `TYPESAFE_API_KEY`) come from the environment or the gitignored `.env`; see `.env.example`. `src/server.py` loads the repo's `.env` by absolute path; the real environment wins.
- One server per data dir: Zvec holds an exclusive lock on the vector store for the server's lifetime. stdio is the default; for concurrent clients run one shared streamable-HTTP server (`ANTAHKARANA_TRANSPORT=http`, `src/transport.py`, README "Running a shared server"). Stop it before `python -m src.dvarapala audit`/`purge`.
