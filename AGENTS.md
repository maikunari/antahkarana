...
- Tests: `python -m pytest` from the repo root (config in `pyproject.toml`). They need no network or API keys; `tests/conftest.py` fakes Gemini, Jev and Zvec. Install `mcp<2`: mcp 2.x removed the `FastMCP` import `src/server.py` uses.
