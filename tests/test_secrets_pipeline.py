...
def _remember(chitta, content, *, model=None, transport=None, **kwargs):
    model = model or FakeOpenRouter(store=True)
    transport = transport or FakeTransport(jev_answer("decision"))
    embeddings = SpyEmbeddings()
    result = tools.remember(
        content,
        chitta=chitta,
        buddhi=make_buddhi(model, make_jev(transport)),
        embeddings=embeddings,
        **kwargs,
    )
    return result, model, transport, embeddings


def _everything_sent(model, transport, embeddings) -> str:
    return json.dumps(
        [model.contents, [c["body"] for c in transport.calls], embeddings.texts]
    )


def test_a_secret_is_scrubbed_before_every_model_call_and_every_write(chitta):
    result, model, transport, embeddings = _remember(chitta, SAMPLE.text, source_agent="pytest")
    ...
    # Buddhi's model, Jev and the embedder saw the scrubbed text, and only that
    scrubbed = SAMPLE.text.replace(SECRET, "[secret:stripe-access-token]")
    assert model.contents == [scrubbed]
    assert transport.calls[0]["body"]["state"]["memory"] == scrubbed
    assert embeddings.texts == [scrubbed]
    assert SECRET not in _everything_sent(model, transport, embeddings)
...
def test_nothing_but_a_secret_is_refused_without_any_model_call(chitta):
    ...
    result, model, transport, embeddings = _remember(chitta, f"STRIPE_KEY={bare}")

    assert result["stored"] is False
    assert result["reason"] == "secret_only"
    assert bare not in json.dumps(result)
    assert model.contents == [] and transport.calls == [] and embeddings.texts == []
...
def test_a_refused_memory_is_logged_scrubbed(chitta):
    result, model, _, _ = _remember(chitta, SAMPLE.text, model=FakeOpenRouter(store=False))

    assert result["stored"] is False
    assert result["redactions"]
    (row,) = determination_rows(chitta)
    assert SECRET not in json.dumps(row)
    assert model.contents and SECRET not in model.contents[0]


def test_a_secret_in_the_scope_or_agent_override_is_dropped(chitta):
    result, model, _, _ = _remember(
        chitta,
    ...
    (record,) = chitta.stored
    assert record.scope == model.answer["scope"]
...
def test_a_secret_echoed_by_buddhi_is_not_stored(chitta):
    ...
    model.answer["scope"] = f"/project/stripe/{SECRET}"
    model.answer["categories"] = ["decision", SECRET]
    ...
    assert SECRET not in json.dumps(result)
    # the log keeps the model's raw answer, scrubbed
    (row,) = determination_rows(chitta)
    assert SECRET not in json.dumps(row)
    assert "[secret:" in row["buddhi"]["response"]["scope"]


def test_remember_fails_closed_when_the_keeper_fails(chitta):
    model, embeddings = FakeOpenRouter(store=True), SpyEmbeddings()
...
def test_chitta_refuses_to_log_a_secret(bare_chitta):
    with pytest.raises(SecretInWrite):
        bare_chitta.log_determination("clean", {"buddhi": {"response": {"scope": SECRET}}})
